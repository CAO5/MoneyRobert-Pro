  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <Bot class="w-6 h-6" style="color: var(--gold)" />
        <h1 class="font-display text-2xl font-bold" style="color: var(--text-primary)">自动交易</h1>
      </div>
      ...
      <div class="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <button @click="form.mode = 'paper'" class="py-2 rounded-lg text-sm" :style="form.mode === 'paper' ? 'background: var(--gold); color: var(--bg-primary)' : 'background: var(--border); color: var(--text-secondary)'">模拟</button>
        ...
        <div>
          ...
          <input v-model.number="form.max_position_size" type="number" class="input-field" />
        </div>
        <div>
          <label class="text-sm mb-1 block" style="color: var(--text-secondary)">最大杠杆</label>
          <input v-model.number="form.max_leverage" type="number" class="input-field" />
        </div>
        <div>
          <label class="text-sm mb-1 block" style="color: var(--text-secondary)">风险比例</label>
          <input v-model.number="form.risk_ratio" type="number" step="0.01" class="input-field" />
        </div>
        <div>
          <label class="text-sm mb-1 block" style="color: var(--text-secondary)">止损 %</label>
          <input v-model.number="form.stop_loss_pct" type="number" class="input-field" />
        </div>
        <div>
          <label class="text-sm mb-1 block" style="color: var(--text-secondary)">止盈 %</label>
          <input v-model.number="form.take_profit_pct" type="number" class="input-field" />
        </div>
        <div>
          <label class="text-sm mb-1 block" style="color: var(--text-secondary)">AI置信度阈值</label>
          <input v-model.number="form.ai_confidence_threshold" type="number" step="0.05" min="0" max="1" class="input-field" />
        </div>
      </div>
    ...
    <template v-else>
      <div class="card">
        ...
        <div v-else class="space-y-3">
          <div v-for="c in configs" :key="c.id" class="flex items-center justify-between p-3 rounded-lg" style="background: var(--bg-primary)">
            <div class="space-y-1">
              <div class="font-medium" style="color: var(--text-primary)">{{ c.symbol }}</div>
              <div class="text-xs space-x-3" style="color: var(--text-muted)">
                <span>模式: <span class="badge" :class="c.mode === 'live' ? 'badge-loss' : 'badge-gold'">{{ c.mode === 'live' ? '实盘' : '模拟' }}</span></span>
                <span>杠杆: {{ c.max_leverage }}x</span>
                <span>风险: {{ c.risk_ratio }}</span>
              </div>
            </div>
            <button @click="toggleConfig(c.id, c.enabled)" class="p-2 rounded-lg transition-colors hover:bg-[#222839]"
      ...
      <div class="card">
        ...
        <div v-else class="space-y-2">
          <div v-for="s in sessions" :key="s.id" class="flex items-center justify-between p-3 rounded-lg" style="background: var(--bg-primary)">
            <div>
