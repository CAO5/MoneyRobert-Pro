...
impl FundManagerAgent {
    ...
    pub async fn make_decision(
        ...
        let weighted_sentiment =
            self.calculate_weighted_sentiment(agent_analyses, &credibility_weights)?;
        let agent_contributions =
            self.calculate_agent_contributions(agent_analyses, &credibility_weights)?;

        let (action, confidence) = self.determine_action(&weighted_sentiment, agent_analyses)?;

        let risk_assessment =
            self.assess_risk(&action, market_snapshot, portfolio_context, agent_analyses)?;

        if !risk_assessment.passed {
            warn!(
                "Risk assessment failed, vetoing decision. Alerts: {:?}",
                risk_assessment.alerts
            );
            return Ok(self.create_hold_decision(
                session_id,
                symbol,
                risk_assessment,
                agent_contributions,
    ...
    fn calculate_credibility_weights(
        &self,
        agent_analyses: &[AgentAnalysis],
    ) -> AgentResult<CredibilityWeights> {
    ...
    fn calculate_weighted_sentiment(
        ...
        for analysis in agent_analyses {
            ...
            *department_sentiments
                .entry(analysis.department.clone())
                .or_insert(0.0) += sentiment_score * agent_weight;
            *department_confidence
                .entry(analysis.department.clone())
                .or_insert(0.0) += analysis.confidence * agent_weight;
        }
    ...
    fn calculate_agent_contributions(
        ...
        for analysis in agent_analyses {
            let contribution_weight = weights
                .agent_weights
                .get(&analysis.agent_name)
                .copied()
                .unwrap_or(0.0);
    ...
    fn assess_risk(
        ...
        if portfolio_context.daily_loss_percent >= self.config.max_daily_loss_percent {
            alerts.push(format!(
                ...
                portfolio_context.daily_loss_percent
            ));
            passed = false;
        }
        ...
        if portfolio_context.consecutive_stop_losses >= 3 {
            alerts.push(format!(
                ...
                portfolio_context.consecutive_stop_losses
            ));
            passed = false;
        }
    ...
    fn calculate_position_parameters(
        ...
        match action {
            DecisionAction::Hold => Ok(PositionParameters {
            ...
            _ => {
                ...
                let entry_price_range = (current_price - price_range, current_price + price_range);
    ...
    fn generate_reasoning(
        ...
        reasoning.push_str(&format!("  - Leverage: {}x\n", position_params.leverage));
        reasoning.push_str(&format!(
            "  - Stop loss: {:.2}%\n",
    ...
    fn update_credibility_weights(&mut self) {
        ...
        for entry in &self.decision_history {
            if let Some(success) = entry.success {
                for contribution in &entry.agent_contributions {
                    let (wins, total) = agent_performance
                        .entry(contribution.agent_name.clone())
                        .or_insert((0, 0));
                    *total += 1;
                    if success {
                        *wins += 1;
        ...
        for (agent_name, (wins, total)) in agent_performance {
            ...
            self.credibility_weights
                .agent_weights
                .insert(agent_name, smoothed_rate);
        }
    }
    ...
    pub fn reflect_on_decisions(&self, lookback_days: i64) -> AgentResult<Vec<Reflection>> {
        ...
        let relevant_decisions: Vec<_> = self
            .decision_history
            .iter()
        ...
        let successful_decisions = relevant_decisions
            .iter()
            .filter(|d| d.success.unwrap_or(false))
            .count();
...
mod tests {
    ...
    fn test_weighted_sentiment_calculation() {
        ...
        let weights = fm.calculate_credibility_weights(&analyses).unwrap();
        let sentiment = fm
            .calculate_weighted_sentiment(&analyses, &weights)
            .unwrap();
    ...
    fn test_risk_assessment() {
        ...
        let risk = fm
            .assess_risk(&DecisionAction::Long, &market, &portfolio, &analyses)
            .unwrap();
        assert!(risk.passed);
    }
    ...
    fn test_position_parameters() {
        ...
        let params = fm
            .calculate_position_parameters(&DecisionAction::Long, 0.75, &market, &risk_assessment)
            .unwrap();
