...
> **重要更新**: 手动调用（用户通过 API 触发）与自动调用（模拟循环自动触发）现在使用完全一致的辩论逻辑和数据采集流程。

#### 一致性保证

| 维度 | 自动调用 | 手动调用 | 一致性说明 |
|------|---------|---------|-----------|
| **市场数据采集** | 包含技术指标、订单簿深度、多时间框架数据 | 包含技术指标、订单簿深度、多时间框架数据 | ✅ 完全一致 |
| **交叉辩论轮次** | Agent 看到对手论据后重新评估 | Agent 看到对手论据后重新评估 | ✅ 完全一致 |
| **历史决策反馈** | 注入历史同类决策的实际结果 | 注入历史同类决策的实际结果 | ✅ 完全一致 |
| **增强置信度计算** | 基于市场上下文、历史准确率、多时间框架对齐度校准 | 基于市场上下文、历史准确率、多时间框架对齐度校准 | ✅ 完全一致 |
| **部门报告提示词** | 关注交叉辩论结果 | 关注交叉辩论结果 | ✅ 完全一致 |
| **基金经理提示词** | 包含可信度、历史决策参考 | 包含可信度、历史决策参考 | ✅ 完全一致 |
...
│                                                                     │
│  Phase 1: 独立分析                                                   │
│  - 每个 Agent 基于统一市场数据进行独立分析                            │
│  - 输出初步观点、置信度、关键论据                                     │
│                                                                     │
│  Phase 2: 交叉辩论轮次（与自动调用一致）                              │
│  - 每个 Agent 看到同部门对手的观点后重新评估                          │
│  - 回应对方最强论据                                                  │
│  - 生成修正后的观点                                                  │
│                                                                     │
│  Phase 3: 部门报告                                                   │
...
│  - 接收三份部门报告 + 辩论纪要                                        │
│  - 注入历史决策反馈（该符号的历史决策胜率）                           │
│  - 计算增强置信度（基于市场上下文、历史准确率、多时间框架对齐度）      │
│  - 输出最终决策（long/short/hold）                                   │
└─────────────────────────────────────────────────────────────────────┘
```

#### 代码实现位置

- **自动调用**: `backend/src/routes/agent_simulation.rs` - `run_debate_for_simulation` 函数
- **手动调用**: `backend/src/routes/ai_analysis.rs` - `start_debate_stream` 函数

两者均调用相同的：
- `fetch_multi_timeframe_data` - 多时间框架数据采集
- `calculate_enhanced_confidence` - 增强置信度计算
- `trends_from_decisions` - 历史决策反馈获取
- 交叉辩论逻辑（Agent 看到对手论据后重新评估）
